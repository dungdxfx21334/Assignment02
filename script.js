...
// I can simply use onClick but it doesn't seem to be challenging enough. ?
  ...
  if (btnDeletes) {
    ...
    for (let j = 0, btnDelLen = btnDeletes.length; j < btnDelLen; j++) {
      ...
      btnDeletes[j].addEventListener("click", function () {
        ...
        // 4 because of the length of "id--" is 4, the rest of the class name is the id of the pet.
...
// For displaying BMI, add a new property to the petAdd[i] called bmi, this property has default value set to "?"
// When the calcBMI is called, the bmi property is modified and the renderTableData() is called to display new value of the bmi property
if (btnBMI) {
...
function calcBMI(petData) {
  ...
  // Use switch in case new type of pets are added in the future.
  switch (petData.type) {
    case "Dog":
      bmi *= 703;
      break;
